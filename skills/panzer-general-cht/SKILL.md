...
5. **★陣營主題(theme)bug — 2026-05-31 徹底改寫,前結論全錯,詳見 `references/ag-ui-runtime.md` §1★**:戰鬥介面 + 購買畫面共用 theme 全域 `[0x5f1b34]`。**實機定案 theme0=盟/theme1=德/theme2=俄**(前筆記寫 ru/ge/al 反了)。兩個本地化引入的 bug:(a) 更早的「配色 fix」把 switch `0x9599d/0x959ac` 對調**改反了方向**(正解 = 原始值 0x9599d+6=00、0x959ac+6=02);(b) **真根因 = classify 的 strcmp 戰役名表(22 名 @0x5F1600)被翻成中文,但場景 lookup 名仍英文 → 永不命中 → classify 死、全部落 bucket2**(這也讓 (a) 的錯誤休眠沒被發現)。正解 = classify 表重指英文副本(顯示表 B' 維持中文)+ switch 改回原始。備份 `.preclassify`/`.pretheme2`。**教訓:strcmp/lookup 字串表絕不可翻;改 theme 前先確認 classify 真的有在比中。**
