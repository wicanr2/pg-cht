...
def decode_rle_v2(d,e):
    off,size=e[2],e[3]
    W=struct.unpack_from(">H",d,off+20)[0]; H=struct.unpack_from(">H",d,off+22)[0]
    pn=bytes(d[off+12:off+16]); chunk_end=off+8+size; cur=off+26; rows=[]
    for r in range(H):
...
MAXRUN_V2=127; MAXLIT_V2=126   # literal cap 126 so 0x80|n never == 0xFF
